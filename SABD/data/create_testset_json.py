"""
Our modified script with only json

## for generating train/valid/test data
python data/create_dataset_our_methodology_json.py --json_file dataset/eclipse/eclipse-recent.json --database eclipse  --bug_data dataset/eclipse/eclipse.json --training dataset/eclipse/training_split_eclipse.txt --validation dataset/eclipse/validation_eclipse.txt --test dataset/eclipse/test_eclipse.txt --date="2020/01/01" --no_tree --dev_perc=0.05

generated `mozillainitial.json`
"""
...
if __name__ == '__main__':
    ...
    help="If this option is enable, so the script checks "
    ...
    fileHandler = logging.FileHandler('./log/create_dataset_{}.log'.format(PROJECT))
    ...
    if args.without_timezone:
        dateThreshold = datetime.strptime(args.date_threshold + \
            " 23:59:59", '%Y/%m/%d %H:%M:%S') if args.date_threshold else None
    else:
        dateThreshold = datetime.strptime(args.date_threshold + \
            " 23:59:59 +0000", '%Y/%m/%d %H:%M:%S %z') if args.date_threshold else None
    ...
    for line in tqdm(cur_latest_json_lines):
        ...
        #not given by our example, if the creationDate of the bug is past our date threshold do not include it in the bug_data
        if dateThreshold is not None and creationDate > dateThreshold:
            nm_report_removed += 1
            continue

        #not given by our example, if it has been selected to not include empty reports and the length of the description/short description is 0
        ...
        #This will only add one copy of the bug to the bug_data, if there is a two bugs of the same description in the bug_data it is already a duplicate
        ...
        #check if there is a dupId present for the current jsonl/bug report
        try:
            dupId = b['dup_id']
        except KeyError:
            dupId = list()
    ...
    if dateThreshold is not None:
        logger.warning(
            "Number of reports that were created after {}: {}".format(args.date_threshold,\
                nm_report_removed))
    ...
    for masterId, masterSet in list(bugsByMasterId.items()):
        if masterId not in bugById:
            ...
            for b in masterSet:
                creationDate = readDateFunc(b)
                # The first bugs have the same date and time of your master. 
                # We check if the id is smaller than the master
            ...
            logging.warning("We didnt find the master report {} that has {} duplicate. \
                New master {}. ".format(masterId, len(masterSet), oldestBug['bug_id']))
    ...
    for dupId, bugList in bugsByMasterId.items():
        ...
        if not args.keep_master:
            ...
            # The report id can be bigger than a report X even though X is newer than first report. 
            # Exchange the id
            ...
            for b in masterSet:
                if int(b['bug_id']) < int(oldestBug):
                    ...
                    logger.warning("Master {} has id bigger than the one of the duplicate reports: \
                        {}. Exchange the ids".format(oldestBug, ids))
    ...
    with codecs.open(args.bug_data, 'w', encoding="utf-8") as f:
